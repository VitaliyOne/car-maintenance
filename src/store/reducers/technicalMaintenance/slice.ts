...
const technicalMaintenancesSlice = createSlice({
  ...
  reducers: {
    ...
    removeTechnicalMaintenancesInfo: (
      state,
      action: PayloadAction<ITechnicalMaintenanceForm['id']>
    ) => {
      const { payload: technicalMaintenancesId } = action;
      state.technicalMaintenances = state.technicalMaintenances.filter(
        (item) => item.id !== technicalMaintenancesId
      );
    }
  }
});
...
export const {
  ...
  deleteTechnicalMaintenancesInfo,
  updateTechnicalMaintenancesInfo,
  removeTechnicalMaintenancesInfo
} = technicalMaintenancesSlice.actions;
