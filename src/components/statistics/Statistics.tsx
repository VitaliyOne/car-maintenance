...
const Statistics = () => {
  ...
  return (
    <section>
      <time className="time" style={{ float: 'left' }}>
        <img src={Stats} alt="iconStatistics" className="icon" /> &nbsp;{dateTime}
      </time>
      {cars && cars.length > 0 ? (
        <h2 className="mt-1r">Данные по: {selectedCar.name}</h2>
      ) : (
        <h2 className="mt-1r">Укажите авто</h2>
      )}
      <NavStatistics />
    </section>
  );
